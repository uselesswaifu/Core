        ...
        if (args.size >= 2) {
            ...
            if (action.equals("create", ignoreCase = true)) {
                if (args.size == 4) {
                    ...
                    if (this.checkSelection(sender)) {
                        if (StorageEngine.getGate(name) == null) {
                            sender.sendMessage(ChatFormat.error("Can't configure gate!"))
                        } else {
                ...
                if (args.size == 4) {
                    ...
                    if (StorageEngine.getGate(name) != null) {
                        sender.sendMessage(ChatFormat.error("Can't configure gate by command in this build."))
                    } else {
                        sender.sendMessage(ChatFormat.error("Gate with name '" + name
                                + "' does not exists!"))
                    }
                } else {
                    sender.sendMessage(ChatFormat.error("Wrong use! Type /gate to help!"))
