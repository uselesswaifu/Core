...
abstract class FakeBaseInventory(holder: InventoryHolder, type: InventoryType, items: Map<Int, Item>, overrideSize: Int?) : BaseInventory(holder, type, items, overrideSize) {
    ...
    private fun inventoryInterface(player: Player) {
        ...
        pk.x = hold?.getX()!!.toInt()
        pk.y = hold.getY().toInt()
        pk.z = hold.getZ().toInt()
        player.dataPacket(pk)
